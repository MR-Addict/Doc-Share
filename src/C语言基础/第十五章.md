...
可见，我们通过 **ptr[i]** 和通过 **num[i]** 来对数组进行访问是一样的，也就是说，我们对数组的操作其实就是对指针的操作，**指针同样支持下标进行访问**，这才下面数组传递给函数体现的更为明显。

# 2. 将数组传递给函数

如果你想要在函数中传递一个一维数组作为参数，你可以通过以下三种方式来声明函数形式参数，这三种声明方式的结果是一样的，因为每种方式都会告诉编译器将要接收一个整型指针。同样地，也可以传递一个多维数组作为形式参数。
...
其实不论哪一种，在哪一种方式中我们将实参传递的时候，都需要将数组的首地址传递过去，所以理论上来讲，传递数组就是传递数组的首地址，通过地址直接访问数组罢了。
...
在这个例子中，bubbleSort(int num[], int length)也可以改成void bubbleSort(int* num, int length)。
...
同理我们可以将函数当作参数进行传递，这称之为`回调函数`，通过回调函数，我们让用户传入自定义的函数，这在中断中使用比较多，日常中有的也很少，这里也不做介绍了，有兴趣可以自行学习。

可见，指针在C语言中的功能真的非常多，有了指针可以实现很多不能实现的功能，大家可以自行参考其他内容进行学习。