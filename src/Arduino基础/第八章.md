...
在Arduino AVR开发板的都只有一个UART，直接和USB连接。当然，你也可以通过使用RX(D0)和TX(D1)进行串口通信，但是，同时只能使用一个，任意一处被占用都无法正常通信。
...
这里只对Arduino的串口通信进行简单介绍，有关UART的通信机制及更多内容，请参考([通讯专题/UART](../../book/通信专题/串口通信/UART/index.html))