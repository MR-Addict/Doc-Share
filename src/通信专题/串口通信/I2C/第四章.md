...
void setAlarm(uint8_t hour, uint8_t minute, bool choice) {
  // 闹钟1
  if (!choice) {
    uint8_t data[] = {
...
bool readAlarmFlag(bool choice) {
...
// 读取温度，我忽略温度的小数位，只读取整数位，从而温度在DS3231中是一个int8_t的数据类型
int8_t readTemp() {
...
void loop() {
  ...
  Serial.print(str);
  if (readAlarmFlag(0))Serial.println("Alarm ON!");
